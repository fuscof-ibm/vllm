...
def stage_postprocess_inputs_to_gpu(
    ...
    index and scheduled/computed/draft token counts into the matching
    pinned numpy views, then issues four non-blocking H→D copies. Buffers
    live on ``ctx`` and only exist when the postprocess kernel is enabled.
    The fused kernel indexes the resulting GPU tensors by ``req_idx``.
